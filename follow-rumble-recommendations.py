__author__ = 'Guillaume Chaslot. Modified for Rumble by Sean House'

"""
    This scripts starts from a search query on rumble and:
...
class RumbleFollower():
    ...
    def get_search_results(self, search_terms, max_results, top_rated=False):
        ...
        url = "https://rumble.com/search/video?q=" + escaped_search_terms
        if self._gl:
            url = url + '&gl=' + self._gl
        ...
        videos = []
        for item_section in soup.findAll('li', {'class': 'video-listing-entry'}):
            video = item_section.contents[0].contents[1]['href']
            videos.append(video)
    ...
    def get_recommendations(self, video_id, nb_recos_wanted, depth, key):
        ...
        # Views
        views = -1
        items = soup.findAll('span', {'class': 'media-heading-info'})
        try:
            views = int(items[1].contents[0].replace(" Views", "").replace(",", ""))
        except IndexError:
            pass

        # Likes
        likes = -1
        for like_count in soup.findAll('span', {'class': 'rumbles-count'}):
            try:
                likes = int(like_count.contents[0].replace(",", ""))
            except IndexError:
                pass
        ...
        pubdate = ""
        for datefield in soup.findAll('span', {'class': 'media-heading-published'}):
            try:
                pubdate = datefield.contents[1]
            except IndexError:
                pass
        ...
        # Recommendations
        recos = []
        reco_items = soup.findAll('li', {'class': "mediaList-item"})
        for video_list in reco_items:
            try:
                recos.append(video_list.contents[1]['href'])
            except IndexError:
        ...
        title = ''
        for eow_title in soup.findAll('h1', {'class': 'h1'}):
        ...
        for video in videos[:max_length]:
            try:
                ...
                print((str(idx) + ') Recommended ' + str(counts[video]) + ' times: '
                    ' https://rumble.com' + video + ' , Title: ' + repr(current_title)))
                if idx % 20 == 0:
                    print('')
                idx += 1
            except KeyError:
...
def compare_keywords(query, search_results, branching, depth, name, gl, language, recent, loopok, alltime):
    ...
    for keyword in query.split(','):
        yf = RumbleFollower(verbose=True, name=keyword, alltime=alltime, gl=gl, language=language, recent=recent, loopok=loopok)
...
def main():
    ...
    parser.add_argument('--gl', help='Location passed to Rumble e.g. US, FR, GB, DE...')
